 ...
 * and the raw OpenAPI spec at /api/docs/openapi.json
 */
...
// Resolve swagger-ui-dist path safely
let swaggerDistPath = null;
try {
  swaggerDistPath = path.dirname(require.resolve("swagger-ui-dist/package.json"));
} catch (_) {
  // swagger-ui-dist not installed — docs will show install instructions
}
...
async function docsHandler(req, res, pathname) {
  ...
  // Static swagger-ui-dist assets
  const assetName = pathname.replace("/api/docs/", "");
  if (ALLOWED_ASSETS[assetName]) {
    if (!swaggerDistPath) {
      res.statusCode = 503;
      res.setHeader("Content-Type", "text/plain");
      res.end("swagger-ui-dist not installed. Run: npm install");
      return true;
    }
    try {
      const filePath = path.join(swaggerDistPath, assetName);
      const content = fs.readFileSync(filePath);
      res.setHeader("Content-Type", ALLOWED_ASSETS[assetName]);
      res.setHeader("Cache-Control", "public, max-age=86400");
      res.statusCode = 200;
      res.end(content);
    } catch (err) {
      console.error("[Swagger asset error]", err.message);
      res.statusCode = 404;
