    ...
    try {
      ...
      if (err.response) {
        ...
        if (status === 403 || status === 429 || status === 503) {
          return errorResponse(res, "THN blocked scraper. RSS fallback also failed. Try again later.", 503);
        }
      ...
      if (err.message && err.message.includes("All RSS sources failed")) {
          return errorResponse(res, "THN scraper blocked and all RSS fallbacks failed. Try again later.", 503);
        }
