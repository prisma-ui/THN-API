  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      const res = await axios.get(url, {
        ...
        maxRedirects: 5,
        decompress: true,
        // Capture full response to inspect headers
        validateStatus: (s) => s < 600,
      });

      // Store any CF cookies from response
      ...
      // Success
      if (status === 200 && !isCFChallenge(html, status)) {
        return html;
      }

      // Cloudflare JS/IUAM challenge — cannot solve without headless browser
      // Throw a specific error so caller knows to use RSS fallback
      if (isCFChallenge(html, status)) {
        const cfErr = new Error("Cloudflare challenge detected — JS challenge cannot be solved without headless browser");
      ...
      err.response = { status, data: html };
      throw err;

    } catch (err) {
      lastErr = err;

      // CF challenge: no point retrying with same method
...
async function scrapeArticleList(pageUrl = BASE_URL) {
  try {
    ...
    // Fallback to RSS for home page on any block/CF challenge
    if (pageUrl === BASE_URL && (isCF || !status)) {
