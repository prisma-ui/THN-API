...
async function scrapeArticle(articleUrl) {
  try {
    ...
    // Extract clean text paragraphs (stop at .note-b)
    const paragraphs = [];
    let stopExtraction = false;
    bodyEl.find("p, div.note-b, .stophere").each((_, el) => {
      if (stopExtraction) return;
      
      const $el = $(el);
      if ($el.hasClass("note-b") || $el.hasClass("stophere")) {
        stopExtraction = true;
        return;
      }
    ...
    // Full HTML content (sanitised and truncated at .note-b)
    const cleanBody = bodyEl.clone();
    cleanBody.find(".note-b, .stophere").nextAll().remove();
    cleanBody.find(".note-b, .stophere").remove();
    const contentHtml = cleanBody.html() || "";
