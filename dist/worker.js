...
function jsonResp(data, status = 200) {
  return new Response(
    JSON.stringify({ success: true, timestamp: (/* @__PURE__ */ new Date()).toISOString(), ...data }, null, 2),
    { status, headers: CORS }
  );
}
function errResp(msg, status = 500) {
  return new Response(
    JSON.stringify({ success: false, error: msg, timestamp: (/* @__PURE__ */ new Date()).toISOString() }),
    { status, headers: CORS }
  );
}
...
function parseArticleCard(html) {
  const url = (html.match(/class="story-link"[^>]*href="([^"]+)"/) || html.match(/href="([^"]+)"[^>]*class="story-link"/))?.[1] || "";
  const title = stripTags(html.match(/<h2[^>]*class="[^"]*home-title[^"]*"[^>]*>([\s\S]*?)<\/h2>/i)?.[1] || html.match(/<h2[^>]*>([\s\S]*?)<\/h2>/i)?.[1] || "");
  ...
  const author = stripTags(html.match(/rel="author"[^>]*>([\s\S]*?)<\/[^>]+>/i)?.[1] || html.match(/class="author-name"[^>]*>([\s\S]*?)<\/[^>]+>/i)?.[1] || "") || null;
...
function buildHeaders(referer = null, cookies = "") {
  ...
  const h = {
    "User-Agent": ua,
    "Accept": isFF ? "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8" : "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8",
...
function isCFChallenge(html, status) {
  ...
  return html.includes("cf-browser-verification") || html.includes("cf_chl_prog") || html.includes("Checking your browser") || html.includes("cloudflare") && html.includes("challenge");
}
function sleep(ms) {
...
function parseRSSXml(xml) {
  ...
  while ((m = itemRe.exec(xml)) !== null) {
    const item = m[1];
    const url = stripTags(item.match(/<link>([\s\S]*?)<\/link>/i)?.[1] || "") || stripTags(item.match(/<feedburner:origLink>([\s\S]*?)<\/feedburner:origLink>/i)?.[1] || "");
    ...
    const image = item.match(/media:content[^>]+url="([^"]+)"/i)?.[1] || item.match(/<enclosure[^>]+url="([^"]+)"/i)?.[1] || null;
    const tags = [];
...
async function scrapeArticleList(pageUrl = BASE_URL) {
  try {
    ...
    const olderMatch = html.match(/id="blog-pager-older-link"[^>]*href="([^"]+)"/i) || html.match(/href="([^"]+)"[^>]*id="blog-pager-older-link"/i);
...
async function scrapeArticle(articleUrl) {
  ...
  const canonicalUrl = getMeta(html, "og:url") || html.match(/<link[^>]+rel="canonical"[^>]+href="([^"]+)"/i)?.[1] || url;
...
async function scrapeSearch(query) {
  ...
  try {
    ...
    const filtered = rss.articles.filter(
  ...
  async fetch(request) {
    ...
    try {
      if (path === "/api/news" || path === "/api") {
        ...
        const data = await scrapeArticleList(pageUrl || void 0);
        setCache(key, data, 5 * 60 * 1e3);
