...
// lib/scraper-cf.js
var BASE_URL = "https://thehackernews.com";
var RSS_URL = "https://feeds.feedburner.com/TheHackersNews";
var RSS_SOURCES = [
...
function buildHeaders(referer = null, extraCookies = "") {
  ...
  const h = {
    "User-Agent": ua,
    "Accept": isFirefox ? "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8" : "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8",
...
function isCFChallenge(html3, status) {
  if (status === 403 || status === 503) return true;
  if (typeof html3 !== "string") return false;
  return html3.includes("cf-browser-verification") || html3.includes("cf_chl_prog") || html3.includes("Checking your browser") || html3.includes("jschl_vc") || html3.includes("cloudflare") && html3.includes("challenge");
}
var cfCookies = {};
...
async function fetchHTML(url, retries = 3) {
  ...
  for (let i = 0; i <= retries; i++) {
    ...
    try {
      ...
      if (isCFChallenge(html3, status)) {
        const e2 = new Error("Cloudflare challenge detected");
        e2.isCFChallenge = true;
        e2.status = status;
        throw e2;
      }
      if (status === 200) return html3;
      const e = new Error(`HTTP ${status}`);
      e.status = status;
      throw e;
    } catch (err2) {
      lastErr = err2;
      if (err2.isCFChallenge) throw err2;
      if (err2.status && err2.status !== 429 && err2.status < 500) throw err2;
    }
  }
  throw lastErr;
}
...
function cleanDateRaw(text3) {
  if (!text3) return "";
  return text3.replace(/^[^A-Za-z0-9]+/, "").trim();
}
function parseDateLabel(text3) {
  if (!text3) return null;
  try {
    const d = new Date(text3);
    if (!isNaN(d.getTime())) return d.toISOString();
  } catch (_) {
  }
  return null;
}
function estimateReadTime(text3) {
  return Math.max(1, Math.ceil(text3.split(/\s+/).filter(Boolean).length / 200));
}
function extractSlug(url) {
  if (!url) return "";
  return url.replace(/^https?:\/\/[^/]+\//, "").replace(/\.html$/, "").replace(/\/$/, "");
}
function parseArticleCard($2, el) {
  const $el = $2(el);
...
function extractNextPageToken($2) {
  const olderLink = $2("a#blog-pager-older-link").attr("href") || $2(".blog-pager-older-link").attr("href") || $2("a:contains('Older Posts')").attr("href") || null;
...
function parseRSSXml(xmlData) {
  const $2 = load(xmlData, { xmlMode: true });
  const articles = [];
  $2("item").each((_, el) => {
    const $el = $2(el);
    ...
    const tags = [];
    $el.find("category").each((_2, cat) => {
      const t = $2(cat).text().trim();
      if (t) tags.push(t);
    });
    const slug = url ? url.replace(BASE_URL + "/", "").replace(/\/$/, "") : "";
...
async function fetchRSS() {
  for (const sourceUrl of RSS_SOURCES) {
    try {
      ...
      if (!res.ok) continue;
      let text3 = await res.text();
      if (sourceUrl.includes("allorigins.win")) {
        try {
          text3 = JSON.parse(text3).contents || text3;
        } catch (_) {
        }
      }
      const result = parseRSSXml(text3);
      if (result.count > 0) return result;
    } catch (err2) {
      console.warn("[THN CF] RSS source failed:", sourceUrl, err2.message);
    }
  }
...
async function scrapeArticleList(pageUrl = BASE_URL) {
  try {
    const html3 = await fetchHTML(pageUrl);
    const $2 = load(html3);
    const articles = [];
    $2("div.body-post").each((_, el) => {
      const a = parseArticleCard($2, el);
      if (a.title && a.url) articles.push(a);
    });
    if (articles.length === 0) {
      $2(".news-container, .story-container").each((_, el) => {
        const a = parseArticleCard($2, el);
        if (a.title && a.url) articles.push(a);
      });
    }
    if (articles.length === 0) return await fetchRSS();
    const pagination = extractNextPageToken($2);
    ...
    const isCF = err2.isCFChallenge || err2.status === 403 || err2.status === 429 || err2.status === 503;
    if (isCF || !err2.status) return await fetchRSS();
    throw err2;
  }
}
async function scrapeArticle(articleUrl) {
  ...
  const $2 = load(html3);
  const metaTitle = $2("meta[property='og:title']").attr("content") || $2("title").text().trim();
  const metaDesc = $2("meta[property='og:description']").attr("content") || $2("meta[name='description']").attr("content") || "";
  const metaImage = $2("meta[property='og:image']").attr("content") || null;
  const metaUrl = $2("meta[property='og:url']").attr("content") || $2("link[rel='canonical']").attr("href") || url;
  const publishedAt = $2("meta[property='article:published_time']").attr("content") || null;
  const modifiedAt = $2("meta[property='article:modified_time']").attr("content") || null;
  const author = $2("[rel='author']").first().text().trim() || $2(".author-name").first().text().trim() || null;
  const tags = [];
  $2(".label a, .post-labels a").each((_, el) => {
    const t = $2(el).text().trim();
    if (t && !tags.includes(t)) tags.push(t);
  });
  const bodyEl = $2("div.articlebody, div.post-body, div#articlebody, div.entry-content").first();
  const paragraphs = [];
  ...
  bodyEl.find("a[href]").each((_, el) => {
    ...
    if (href && !href.startsWith("#") && !href.includes("thehackernews.com") && href.startsWith("http")) links.push({ text: text3, url: href });
  });
  const images = [];
  bodyEl.find("img").each((_, el) => {
    const src = $2(el).attr("data-src") || $2(el).attr("data-original") || $2(el).attr("src") || "";
  ...
  bodyEl.find("h2, h3, h4").each((_, el) => {
    headings.push({ level: el.tagName.toLowerCase(), text: $2(el).text().trim() });
  });
  return {
    url: metaUrl,
    ...
    tags,
    read_time_minutes: estimateReadTime(paragraphs.join(" ")),
...
function setCache(key, data2, ttl) {
  CACHE.set(key, { data: data2, exp: Date.now() + ttl });
}
var CORS = {
...
function json(data2, status = 200) {
  return new Response(JSON.stringify({ success: true, timestamp: (/* @__PURE__ */ new Date()).toISOString(), ...data2 }, null, 2), {
    status,
    headers: CORS
  });
}
function err(msg, status = 500) {
  return new Response(JSON.stringify({ success: false, error: msg, timestamp: (/* @__PURE__ */ new Date()).toISOString() }), {
    status,
    headers: CORS
  });
}
  ...
  async fetch(request, env) {
    ...
    try {
      if (path === "/api/news" || path === "/api") {
        ...
        const data2 = await scrapeArticleList(pageUrl || void 0);
        setCache(key, data2, 5 * 60 * 1e3);
        return json({ cached: false, ...data2 });
      }
      if (path === "/api/article") {
        ...
        setCache(key, article, 30 * 60 * 1e3);
      ...
      if (catMatch) {
        ...
        const data2 = await scrapeCategory(name, cursor);
        setCache(key, data2, 5 * 60 * 1e3);
        return json({ cached: false, category: name, ...data2 });
      }
      if (path === "/api/search") {
        ...
        const data2 = await scrapeSearch(q.q);
        setCache(key, data2, 2 * 60 * 1e3);
        return json({ cached: false, ...data2 });
      }
      if (path === "/api/meta") {
        const cached = getCache("meta");
        if (cached) return json({ cached: true, ...cached });
        const data2 = await scrapeSiteMeta();
        setCache("meta", data2, 60 * 60 * 1e3);
        return json({ cached: false, ...data2 });
      }
